...
import { MapPin, Play, Trophy, Calendar, Clock, Award, ArrowUpRight, ArrowRight } from "lucide-react";
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {results.map((r) => (
                        <div
                            key={r.id}
                            className={`relative rounded-lg p-3 border-2 shadow-sm ${r.winner
                        ...
                        >
                            ...
                            <div className="flex justify-between items-center text-sm font-bold mb-2">
                                <span>{r.team1} {r.score1 && `(${r.score1})`}</span>
                                <span className="text-slate-400">VS</span>
                                <span>{r.team2} {r.score2 && `(${r.score2})`}</span>
                            </div>
                            ...
                            {r.winner ? (
                                ...
                                <p className="text-xs text-slate-500 mb-2">🏏 Match Coming Soon</p>
                            )}

                            {/* Action */}
                            <button
                                disabled={!r.highlightLink}
                                onClick={() => r.highlightLink && window.open(r.highlightLink, "_blank")}
                                className="
